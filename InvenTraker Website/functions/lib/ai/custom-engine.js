...
const providerFromEnv = (process.env.INVENTRAKER_AI_PROVIDER?.trim().toLowerCase() || "custom-rules");
function aiProvider() {
    if (providerFromEnv === "openai-chat")
        return "openai-chat";
    return "custom-rules";
}
...
async function callOpenAiJson(intent, systemPrompt, userPrompt) {
    ...
    if (!content)
        throw new Error(`No content from model for ${intent}.`);
...
function buildHeuristicOrderInsights(lines) {
    const risky = lines
...
export async function enhanceHowToDraft(input) {
    ...
    if (aiProvider() !== "openai-chat")
        return fallback;
    try {
        const systemPrompt = "You are InvenTraker's internal AI engine. Improve procedural instructions while preserving exact operational meaning.";
        ...
        const { payload, model } = await callOpenAiJson("pdf_howto_draft", systemPrompt, userPrompt);
...
export async function enhanceOrderSuggestions(input) {
    ...
    if (aiProvider() !== "openai-chat" || input.lines.length === 0)
        return fallback;
    try {
        const systemPrompt = "You are InvenTraker's internal ordering AI. Improve rationale text and produce concise risk alerts for managers.";
        const userPrompt = JSON.stringify({
        ...
        const { payload, model } = await callOpenAiJson("order_suggestions", systemPrompt, userPrompt);
        const rationaleById = new Map((payload.lines ?? [])
            .filter((line) => typeof line.itemId === "string" && typeof line.rationale === "string")
...
function buildHeuristicFinancialNarrative(snapshot) {
    const riskAlerts = [];
...
export async function enhanceFinancialHealth(snapshot) {
    ...
    if (aiProvider() !== "openai-chat")
        return fallback;
    try {
        const systemPrompt = "You are InvenTraker's internal finance operations AI. Produce practical insights from inventory health metrics.";
        const userPrompt = JSON.stringify({
        ...
        const { payload, model } = await callOpenAiJson("financial_health", systemPrompt, userPrompt);
        return {
            summary: typeof payload.summary === "string" && payload.summary.trim() ? payload.summary.trim() : heuristics.summary,
