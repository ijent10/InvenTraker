...
export const listMyOrganizations = onCall(async (request) => {
    ...
    for (const orgDoc of orgsSnap.docs) {
        ...
        const locationIds = Array.isArray(memberData.locationIds)
            ...
            : [];
        const permissionFlags = role === "Owner"
...
export const getStoreRecommendations = onCall({ timeoutSeconds: 180, memory: "1GiB" }, async (request) => {
    ...
    if (!parsed.success) {
        throw new HttpsError("invalid-argument", "Invalid recommendation preview request.", parsed.error.issues.map((issue) => ({ path: issue.path.join("."), message: issue.message })));
    }
...
export const commitOrderRecommendations = onCall({ timeoutSeconds: 120, memory: "1GiB" }, async (request) => {
    ...
    if (!parsed.success) {
        throw new HttpsError("invalid-argument", "Invalid recommendation commit request.", parsed.error.issues.map((issue) => ({ path: issue.path.join("."), message: issue.message })));
    }
    const input = parsed.data;
    try {
        await requireStoreAccess(input.orgId, uid, input.storeId);
...
export const generateOrderSuggestions = onCall({ timeoutSeconds: 180, memory: "1GiB" }, async (request) => {
    ...
    if (!parsed.success) {
        throw new HttpsError("invalid-argument", "Invalid generate-order request.", parsed.error.issues.map((issue) => ({ path: issue.path.join("."), message: issue.message })));
    }
    const input = parsed.data;
    try {
        await requireStoreAccess(input.orgId, uid, input.storeId);
