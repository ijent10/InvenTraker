...
function chooseCategory(raw: Record<string, unknown>): string | null {
  const direct =
    asStringOrNull(raw.generalizedCategoryKey) ??
...
function mapCatalogResponse(barcode: string, raw: Record<string, unknown>) {
  const canonicalName =
    asStringOrNull(raw.canonicalName) ??
...
export const simpliPantriCatalogLookup = onRequest(async (request, response) => {
  ...
  const barcodeInput =
    ...
    (request.body && typeof request.body === "object" ? (request.body as Record<string, unknown>).barcode : null)
  ...
  for (const candidate of candidates) {
    ...
    if (globalSnap?.exists) {
      const raw = (globalSnap.data() as Record<string, unknown> | undefined) ?? {}
    ...
    if (legacySnap?.exists) {
      const raw = (legacySnap.data() as Record<string, unknown> | undefined) ?? {}
      response.status(200).json({
        found: true,
    ...
    if (intakeSnap?.exists) {
      const raw = (intakeSnap.data() as Record<string, unknown> | undefined) ?? {}
      response.status(200).json({
        found: true,
...
export const simpliPantriCatalogEnrich = onRequest(async (request, response) => {
  ...
  const entries: SimpliPantriCatalogPayload[] = Array.isArray(rawBody)
    ? (rawBody as SimpliPantriCatalogPayload[])
    : Array.isArray((rawBody as { products?: unknown } | null)?.products)
      ? (((rawBody as { products?: unknown }).products ?? []) as SimpliPantriCatalogPayload[])
      : []
  ...
  const dedupedByBarcode = new Map<string, SimpliPantriCatalogPayload>()
  ...
  for (const [barcode, entry] of dedupedByBarcode.entries()) {
    const canonicalName =
      asStringOrNull(entry.canonicalName) ??
