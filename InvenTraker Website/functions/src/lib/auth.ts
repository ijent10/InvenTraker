...
function normalizeRole(rawRole: unknown, ownerByArray: boolean): "Owner" | "Manager" | "Staff" {
...
export function requireAuth(request: AuthedRequest): string {
  const uid = request.auth?.uid
  if (!uid) throw new HttpsError("unauthenticated", "Authentication required")
  return uid
}

export async function requirePlatformAdmin(request: AuthedRequest): Promise<string> {
  const uid = requireAuth(request)
  const isPlatformAdmin = request.auth?.token?.platform_admin === true
  if (isPlatformAdmin) return uid
  ...
  const userData =
    (userSnap.data() as { platformRoles?: { platformAdmin?: boolean }; email?: string } | undefined) ?? {}
...
export async function getMembership(orgId: string, uid: string) {
  ...
  if (!member && !ownerByArray) return null
...
export async function requireOrgMembership(orgId: string, uid: string) {
  const member = await getMembership(orgId, uid)
  if (!member) throw new HttpsError("permission-denied", "Not a member of this organization")
  return member
}

export async function requireStoreAccess(orgId: string, uid: string, storeId: string) {
...
): Promise<Awaited<ReturnType<typeof requireOrgMembership>>> {
  const member = await requireOrgMembership(orgId, uid)
  if (member.role === "Owner") return member
  if (member.permissionFlags?.[permissionKey] === true) return member
