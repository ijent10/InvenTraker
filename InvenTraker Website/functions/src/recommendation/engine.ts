...
import {
  loadCachedRecommendationSnapshot,
  persistRecommendationArtifacts,
  recommendationInputHash
} from "./persistence.js"
import { collectRecommendationQuestions, topGlobalDrivers } from "./rationale.js"
import type { CollectedRecommendationFeatures } from "./types.js"

function normalizeDomains(domains?: RecommendationDomain[]): RecommendationDomain[] {
...
function parseDateOrNull(value: string | undefined): Date | undefined {
  if (!value) return undefined
  const parsed = new Date(value)
  return Number.isNaN(parsed.getTime()) ? undefined : parsed
}

export type BuildStoreRecommendationInput = {
  orgId: string
  storeId: string
  actorUid: string
  vendorId?: string
...
export type BuildStoreRecommendationResult = {
  response: GetStoreRecommendationsResponse
  features: CollectedRecommendationFeatures
}

export async function buildStoreRecommendations(
  input: BuildStoreRecommendationInput
): Promise<BuildStoreRecommendationResult> {
  ...
  try {
    ...
    const demandByItem = new Map<string, DemandPrediction>()
    const wasteRiskByItem = new Map<string, WasteRiskPrediction>()
    ...
    const response: GetStoreRecommendationsResponse = {
      meta: {
        runId,
...
export function summarizeRecommendationDrivers(response: GetStoreRecommendationsResponse) {
