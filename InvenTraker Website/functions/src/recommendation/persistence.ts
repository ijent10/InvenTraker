...
import type { GetStoreRecommendationsResponse, RecommendationDomain } from "@inventracker/shared"
...
function recommendationsCollectionPath(features: CollectedRecommendationFeatures): string {
  return `${features.storePath.path}/recommendations`
}

function recommendationRunsCollectionPath(features: CollectedRecommendationFeatures): string {
  return `${features.storePath.path}/recommendationRuns`
}

export function recommendationInputHash(features: CollectedRecommendationFeatures): string {
  const payload = {
...
export async function loadCachedRecommendationSnapshot(features: CollectedRecommendationFeatures) {
  const key = windowKey({
