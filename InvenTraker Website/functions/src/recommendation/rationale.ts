import type { OrderRecommendation, ProductionRecommendation, RecommendationDriver } from "@inventracker/shared"

export function dedupeQuestions(input: Array<string | undefined>): string[] {
...
export function topGlobalDrivers(input: {
  ...
  const scoreByKey = new Map<string, { totalImpact: number; count: number; sample: RecommendationDriver }>()

  const add = (driver: RecommendationDriver) => {
  ...
  for (const row of input.orderRecommendations) {
    for (const driver of row.topContributingFactors) add(driver)
  }
