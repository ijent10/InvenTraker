import type { WasteRiskPrediction } from "@inventracker/shared"
import type { ItemFeature } from "./types.js"

export function runWasteRiskRulesV1(item: ItemFeature): WasteRiskPrediction {
  ...
  const probability = Math.min(
    0.95,
    Math.max(0.03, wasteRatio * 0.45 + expiringRatio * 0.4 + Math.min(0.2, demandCoverage * 0.05))
  )
