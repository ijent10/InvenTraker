...
import { AppButton, AppCard, AppInput, DataTable, type TableColumn } from "@inventracker/ui"
...
export default function AdminInventoryDatabasePage() {
  ...
  const columns: TableColumn<CentralCatalogItemRecord>[] = [
    ...
    {
      key: "exp",
      header: "Default Expiration",
      render: (row) => `${Number(row.defaultExpirationDays ?? 0)} days`
    },
    {
      key: "actions",
      header: "Actions",
  ...
  const save = async () => {
    ...
    try {
      await upsertCentralCatalogItem({
        ...
        defaultExpirationDays: Number(defaultExpirationDays || "0")
      })
      await refetch()
      setEditingId("")
      setUpc("")
    ...
    try {
      ...
      for (const item of orgItems) {
        ...
        await upsertCentralCatalogItem({
          ...
          upc: barcode,
          name: item.name,
          defaultExpirationDays: Number(item.defaultExpirationDays ?? 0)
        })
        imported += 1
      }
      await refetch()
