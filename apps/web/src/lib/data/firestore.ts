...
function normalizeItemRecord(
  ...
  const defaultExpiration = Math.max(1, asNumber(data.defaultExpirationDays ?? data.defaultExpiration, 7))
...
function organizationItemPatch(patch: Partial<ItemRecord>): Record<string, unknown> {
  ...
  if (patch.defaultExpirationDays !== undefined || patch.defaultExpiration !== undefined) {
    const expiration = Math.max(1, asNumber(patch.defaultExpirationDays ?? patch.defaultExpiration, 7))
    record.defaultExpirationDays = expiration
    record.defaultExpiration = expiration
  }
  if (patch.defaultPackedExpiration !== undefined) {
    record.defaultPackedExpiration = Math.max(1, asNumber(patch.defaultPackedExpiration, 1))
  }
  ...
  if (globalSnap) {
    for (const docSnap of globalSnap.docs) {
      ...
      merged.set(docSnap.id, {
        ...
        defaultExpirationDays: Math.max(1, asNumber(data.defaultExpiration ?? data.defaultExpirationDays, 7)),
        updatedAt: data.updatedAt
      })
    }
  }
  ...
  if (legacySnap) {
    for (const docSnap of legacySnap.docs) {
      ...
      merged.set(docSnap.id, {
        ...
        photoAssetId: asString(data.photoAssetId),
        thumbnailBase64: asString(data.thumbnailBase64),
        defaultExpirationDays: Math.max(1, asNumber(data.defaultExpirationDays ?? data.defaultExpiration, 7)),
        updatedAt: data.updatedAt
      })
    }
  }
...
export async function upsertCentralCatalogItem(
  ...
  const payload = {
    ...
    defaultExpiration: Math.max(1, Number(item.defaultExpirationDays ?? 7)),
    defaultExpirationDays: Math.max(1, Number(item.defaultExpirationDays ?? 7)),
    updatedAt: serverTimestamp()
  }
  await setDoc(
    globalRef,
