...
function toStepTitle(stepNumber) {
    return `Step ${stepNumber}`;
}
function isPureStepNumberLine(line) {
    if (!/^\d{1,2}$/.test(line))
        return null;
...
function parseStructuredNumberedLines(lines) {
    const rawSteps = [];
    let expected = 1;
    let pendingQuantity = null;
    for (const line of lines) {
        if (/recipe build sheets/i.test(line))
            break;
...
function parseInlineNumberedText(text) {
    const flattened = cleanText(text).replace(/\n+/g, " ");
    const unitGuard = "(?:oz|lb|lbs|ea|each|ct)\\b";
    const regex = new RegExp(String.raw `(?:^|\s)(\d{1,2})(?:[.)])?\s+(?!${unitGuard})([\s\S]*?)(?=(?:\s\d{1,2}(?:[.)])?\s+(?!${unitGuard}))|$)`, "gi");
    const candidates = [];
...
export function buildHowToDraftFromText(text, titleHint) {
...
export async function extractHowToDraftFromPdf(buffer) {
