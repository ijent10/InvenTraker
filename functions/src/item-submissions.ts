...
const draftSchema = z.object({
  ...
  defaultExpirationDays: z.number().int().min(1).max(730).optional(),
  defaultPackedExpiration: z.number().int().min(1).max(730).optional(),
...
function normalizeUpc(raw: string | undefined): string | null {
  if (!raw) return null
...
function normalizeItemDraft(input: DraftInput, fallbackUpc?: string): Record<string, unknown> {
  ...
  return {
    ...
    defaultExpirationDays: Math.max(1, input.defaultExpirationDays ?? 7),
    defaultPackedExpiration: Math.max(1, input.defaultPackedExpiration ?? input.defaultExpirationDays ?? 7),
...
function resolveItemDocId(draft: Record<string, unknown>): string {
...
export const reviewItemSubmission = onCall(async (request) => {
  ...
  const rawDraft = (submission.itemDraft as Record<string, unknown> | undefined) ?? null
