...
function roleAliases(role: string, jobTitle?: string): Set<string> {
  const aliases = new Set<string>()
  const normalizedRole = normalizeText(role)
  if (normalizedRole) aliases.add(normalizedRole)
...
function memberMatchesRoleTargets(
  ...
  for (const alias of aliases) {
    if (normalizedTargets.has(alias)) return true
  }
  return false
}
...
async function resolveRecipientUserIds(input: {
  ...
  for (const ownerUid of orgOwnerIds) {
    ...
    if (
      memberMatchesRoleTargets(normalizedTargets, "owner", "owner")
      || normalizedTargets.has("all")
      || normalizedTargets.has("everyone")
      || normalizedTargets.has("team")
    ) {
...
async function loadRecipientDeviceTokens(userIds: string[]) {
  const tokenToDocPath = new Map<string, string>()
  for (const uid of userIds) {
    const devicesSnap = await adminDb.collection("users").doc(uid).collection("devices").get()
    for (const deviceDoc of devicesSnap.docs) {
      const data = deviceDoc.data() as { fcmToken?: string; disabled?: boolean }
...
async function sendPushForNotification(input: {
  ...
  const staleDocPaths = new Set<string>()
  let sent = 0
  let failed = 0
  const chunkSize = 500
  for (let offset = 0; offset < tokens.length; offset += chunkSize) {
    ...
    response.responses.forEach((entry, index) => {
      ...
      if (
        ...
        if (docPath) staleDocPaths.add(docPath)
      }
    })
  }
...
export const removeOrgNotification = onCall(async (request) => {
  ...
  if (data?.dispatchMode !== "scheduled" || data?.status !== "queued") {
    throw new HttpsError(
      "failed-precondition",
      "Only queued scheduled notifications can be removed."
    )
  }
