import type { OrderRecommendation, RecommendationDriver, DemandPrediction, WasteRiskPrediction } from "@inventracker/shared"
import type { ItemFeature } from "./types.js"

function confidenceForItem(item: ItemFeature): number {
  let score = 0.35
  if (item.weeklyUsage > 0) score += 0.2
  if (item.wasteAffectingOrders > 0) score += 0.15
  if (item.expiringBeforeLead > 0) score += 0.1
  if (item.incomingBeforeLead > 0) score += 0.1
  if (item.vendorId) score += 0.1
  if (item.productionDemand > 0) score += 0.1
  return Number(Math.min(0.95, score).toFixed(3))
}

function buildTopFactors(item: ItemFeature, demand: DemandPrediction, wasteRisk: WasteRiskPrediction): RecommendationDriver[] {
  return [
    {
      key: "on_hand",
...
export function runOrderOptimizerRulesV1(input: {
  items: ItemFeature[]
  demandByItem: Map<string, DemandPrediction>
  wasteRiskByItem: Map<string, WasteRiskPrediction>
}): OrderRecommendation[] {
  const rows: OrderRecommendation[] = []
