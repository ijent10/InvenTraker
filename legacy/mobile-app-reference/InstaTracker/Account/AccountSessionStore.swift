...
final class AccountSessionStore: ObservableObject {
    ...
    private var lastAccessContextRefreshAt: Date = .distantPast
    #if canImport(FirebaseFirestore)
    ...
    func updatePassword(currentPassword: String, newPassword: String) async {
        do {
            ...
            // Align with web-style security posture: require a fresh login after credential changes.
            signOut()
            errorMessage = "Password updated. Please sign in again."
        } catch {
            errorMessage = error.localizedDescription
        }
        isLoading = false
    ...
    func revalidateAuthSession() async {
        guard firebaseUser != nil else { return }
        do {
            let stillValid = try await authService.validateCurrentSession()
            if !stillValid {
                signOut()
            }
        } catch {
    ...
    func refreshAccessContextIfNeeded(
        force: Bool = false,
        minInterval: TimeInterval = 45
    ) async {
        guard firebaseUser != nil else { return }
        let now = Date()
        if !force, now.timeIntervalSince(lastAccessContextRefreshAt) < minInterval {
            return
        }
        await refreshMembershipContext(showLoading: false)
        lastAccessContextRefreshAt = Date()
    }
    ...
    private func refreshMembershipContext(showLoading: Bool = true) async {
        guard let user = firebaseUser else {
            stopMembershipListener()
            organizations = []
            stores = []
    ...
    private func configureMembershipListener(userId: String, organizationId: String) {
        ...
        membershipListener = ref.addSnapshotListener { [weak self] _, _ in
            Task { @MainActor in
                await self?.refreshMembershipContext(showLoading: false)
            }
        }
#else
        _ = userId
