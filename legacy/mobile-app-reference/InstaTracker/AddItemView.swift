...
struct AddItemView: View {
    ...
    private func saveItem() async {
        ...
        if !normalizedUPC.isEmpty,
           existingCatalogProduct == nil,
           !session.canPerform(.manageCatalog) {
            await submitUnknownUPCForReview(
                upc: normalizedUPC,
                item: item
            )
        }
    ...
    @MainActor
    private func submitUnknownUPCForReview(
        upc: String,
        item: InventoryItem
    ) async {
        let submissionId = await catalogService.submitItemDraftForVerification(
            ...
            scannedUPC: upc,
            draftPayload: CatalogInventoryImporter.submissionDraftPayload(from: item),
            note: "Created from Add Item unknown scan."
        )
        catalogLookupMessage = submissionId == nil
            ? "Saved item locally. Central review submission could not be sent right now."
            : "Saved item locally and sent it for catalog review."
    }
