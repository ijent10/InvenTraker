...
struct ReceivedView: View {
    ...
    private func createUnknownStoreDraftAndContinue(scannedUPC normalizedUPC: String) {
        ...
        let trimmedUPC = (draftItem.upc ?? "").trimmingCharacters(in: .whitespacesAndNewlines)
        if session.canPerform(.manageCatalog), !trimmedUPC.isEmpty {
    ...
    private func continuePendingUnknownDraftFlow() {
        ...
        requiresMinimumSetupForScannedItem = shouldPromptMinimumSetup(for: draft)
        scannedItem = draft
    }
