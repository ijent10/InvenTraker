...
enum CentralCatalogError: LocalizedError {
    ...
    var errorDescription: String? {
        ...
        case .missingStore:
            return "An active store is required."
        case .missingPermission:
...
final class CentralCatalogService {
            ...
            if firestoreEnabled {
                ...
                if let organizationId, !organizationId.isEmpty {
                    ...
                    if let storeId, !storeId.isEmpty {
                        storeSnapshot = try? await fetchStoreCatalogDocument(
    ...
    func submitItemDraftForVerification(
        ...
        guard isSyncableOrganizationId(normalizedOrgID),
              !normalizedStoreID.isEmpty,
              !normalizedUID.isEmpty else { return nil }
    ...
    func upsertCompanyProductWithoutUPC(
        ...
        guard let editorOrganizationId, isSyncableOrganizationId(editorOrganizationId) else {
            throw CentralCatalogError.missingOrganization
        }
        let normalizedStoreID = storeId.trimmingCharacters(in: .whitespacesAndNewlines)
        guard !normalizedStoreID.isEmpty else {
            throw CentralCatalogError.missingStore
        }
    ...
    func upsertProduct(
        ...
        if firestoreEnabled {
            ...
            let legacyStoreRef = db.collection("organizations")
                ...
                .collection("catalog")
                .document(upc)
            try batch.setData(from: store, forDocument: legacyStoreRef)

            if let canonicalStoreRef = await resolveCanonicalStoreDocumentReference(
                organizationId: editorOrganizationId,
                storeId: normalizedStoreID,
                db: db
            ) {
                let canonicalStoreCatalogRef = canonicalStoreRef
                    .collection("catalog")
                    .document(upc)
                if canonicalStoreCatalogRef.path != legacyStoreRef.path {
                    try batch.setData(from: store, forDocument: canonicalStoreCatalogRef)
                }
            }
    ...
    private func sanitizeStoreIdentifier(_ rawStoreID: String) -> String {
        let trimmed = rawStoreID.trimmingCharacters(in: .whitespacesAndNewlines)
    ...
    private func resolveCanonicalStoreDocumentReference(
        ...
        let legacyStoreRef = orgRef.collection("stores").document(normalizedStoreID)
    ...
    private func fetchStoreCatalogDocument(
        organizationId: String,
        storeId: String,
        upc: String,
        db: Firestore
        ...
        let legacySnapshot = try? await db.collection("organizations")
            .document(organizationId)
            .collection("stores")
            .document(normalizedStoreID)
            .collection("catalog")
        ...
        if let canonicalStoreRef = await resolveCanonicalStoreDocumentReference(
            ...
            let canonicalSnapshot = try? await canonicalStoreRef
                .collection("catalog")
                .document(upc)
                .getDocument()
            if canonicalSnapshot?.exists == true {
