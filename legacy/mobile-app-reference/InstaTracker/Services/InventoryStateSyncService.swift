...
final class InventoryStateSyncService: InventoryStateSyncing {
    ...
    private var canonicalStoreReferenceCache: [String: (path: String?, cachedAt: Date)] = [:]
    private let canonicalStoreReferenceCacheTTL: TimeInterval = 300
    ...
    private var realtimeTodoLegacyListener: ListenerRegistration?
    private var realtimeWasteListener: ListenerRegistration?
    private var realtimeWasteRecordsListener: ListenerRegistration?
    #endif
    private var remoteImageCache: [String: Data] = [:]
    private var remoteImageCacheOrder: [String] = []
    private let remoteImageCacheLimit = 16
    ...
    func startRealtimeInventorySync(
        ...
        guard isSyncableOrganizationId(organizationId) else { return }
        ...
        if listeningOrganizationId == organizationId, listeningStoreId == storeId,
           ...
           realtimeOrdersListener != nil, realtimeToDoListener != nil, realtimeTodoLegacyListener != nil, realtimeWasteListener != nil,
           realtimeWasteRecordsListener != nil {
            return
        }
        ...
        realtimeTodoLegacyListener = Firestore.firestore()
            .collectionGroup("todo")
            .whereField("organizationId", isEqualTo: organizationId)
            .whereField("storeId", isEqualTo: storeId)
            .addSnapshotListener { [weak self] snapshot, error in
        ...
        realtimeWasteRecordsListener = Firestore.firestore()
    ...
    func stopRealtimeInventorySync() {
        ...
        realtimeTodoLegacyListener?.remove()
        realtimeWasteListener?.remove()
        realtimeWasteRecordsListener?.remove()
        ...
        realtimeTodoLegacyListener = nil
        ...
        canonicalStoreReferenceCache.removeAll(keepingCapacity: false)
    ...
    func refreshStoreScopeFromRemote(
        ...
        guard isSyncableOrganizationId(normalizedOrganizationID) else { return false }
...
#if canImport(FirebaseFirestore)
        guard remoteSyncAvailable else { return }
        guard isSyncableOrganizationId(refs.organizationId) else { return }
    ...
    func refreshProductionCacheFromRemote(
        ...
        guard remoteSyncAvailable else { return false }
        guard isSyncableOrganizationId(organizationId) else { return false }
        let db = Firestore.firestore()
    ...
    func pullInventoryCacheFromRemote(
        ...
        let orgRef = db.collection("organizations").document(organizationId)
        var didMerge = false
        let localPendingSpotCheckItemIDs = pendingSpotCheckItemIDs(
            organizationId: organizationId,
            modelContext: modelContext
        )
        ...
        let vendorDescriptor = FetchDescriptor<Vendor>(
            predicate: #Predicate { $0.organizationId == organizationId }
        )
        ...
        if let vendorSnapshot {
            var materializedRemoteVendorBackendIDs: Set<String> = []
            var materializedRemoteVendorUUIDs: Set<UUID> = []
            for doc in vendorSnapshot.documents {
                ...
                materializedRemoteVendorBackendIDs.insert(backendId)
                ...
                materializedRemoteVendorUUIDs.insert(vendor.id)
            }

            // Keep local vendor state aligned with Firestore by pruning org vendors that no longer exist remotely.
            for vendor in existingVendors {
                let backendId = (vendor.backendId ?? "").trimmingCharacters(in: .whitespacesAndNewlines)
                let matchesRemoteBackend = !backendId.isEmpty && materializedRemoteVendorBackendIDs.contains(backendId)
                let matchesRemoteUUID = materializedRemoteVendorUUIDs.contains(vendor.id)
                guard !matchesRemoteBackend, !matchesRemoteUUID else { continue }
                modelContext.delete(vendor)
                vendorByUUID.removeValue(forKey: vendor.id)
                if !backendId.isEmpty {
                    vendorByBackendID.removeValue(forKey: backendId)
                }
                didMerge = true
            }
        }
        ...
        if let itemSnapshot {
            ...
            if let storeOverrideSnapshot = try? await orgRef
                ...
                for overrideDoc in storeOverrideSnapshot.documents {
                    ...
                    let overrideStoreID = sanitizeStoreIdentifier((overrideData["storeId"] as? String) ?? "")
                    if overrideStoreID != normalizedStoreID {
                        continue
                    }
            ...
            for item in existingItems {
                let scopedStore = sanitizeStoreIdentifier(item.storeId)
                guard scopedStore == normalizedStoreID else { continue }
            ...
            for doc in itemSnapshot.documents {
                ...
                let remoteStoreRaw = ((data["storeId"] as? String) ?? "")
                    .trimmingCharacters(in: .whitespacesAndNewlines)
                let remoteStoreID = sanitizeStoreIdentifier(remoteStoreRaw)
                   ...
                   let embedded = data["batches"] as? [[String: Any]] {
                    let embeddedScoped = embedded.enumerated().compactMap { index, raw -> [String: Any]? in
                        ...
                        let resolvedStoreForBatch: String = {
                            let normalizedBatchStore = sanitizeStoreIdentifier(batchStore)
                            if !normalizedBatchStore.isEmpty { return normalizedBatchStore }
                            if !remoteStoreID.isEmpty { return remoteStoreID }
                            return ""
                        }()
                        guard !resolvedStoreForBatch.isEmpty, resolvedStoreForBatch == normalizedStoreID else {
                            return nil
                        }
                        var normalized = raw
                ...
                if !remoteStoreID.isEmpty, remoteStoreID != normalizedStoreID {
                    continue
                }
                let resolvedRemoteStore = remoteStoreID.isEmpty ? normalizedStoreID : remoteStoreID
                ...
                let shouldMaterializeForStore = !remoteStoreID.isEmpty || hasStoreScopedQuantity || hasExplicitStoreAssignment
                ...
                if item.storeId != resolvedRemoteStore {
                    item.storeId = resolvedRemoteStore
                    itemChanged = true
                }
                if
                    !itemChanged,
                ...
                let hasPendingSpotCheck = localPendingSpotCheckItemIDs.contains(item.id)
                if hasPendingSpotCheck {
                    // Keep local spot-check quantities while outbound sync is pending/failed.
                    // Without this, stale remote snapshots can overwrite freshly counted stock.
                } else if batchesDiffer(local: item.batches, remote: scopedBatches) {
                ...
                if hasRemoteImagePointer || item.pictures.isEmpty {
                    if let remoteImage = await resolveRemoteImageData(data: data, cacheKey: "\(organizationId)|\(backendId)") {
                        if item.pictures.first != remoteImage {
                            item.pictures = [remoteImage]
                            itemChanged = true
                        }
                    }
                }
            ...
            let scopedExistingItems = existingItems.filter { item in
                ...
                sanitizeStoreIdentifier(item.storeId) == normalizedStoreID
            }
    ...
    private func mergeStoreInventoryFromBatchesOnly(
        ...
        for (itemId, remoteBatches) in batchesByItemId {
            ...
            let hasPendingSpotCheck = localPendingSpotCheckItemIDs.contains(item.id)
            if hasPendingSpotCheck {
                // Preserve local counts until spot-check sync settles.
            } else if batchesDiffer(local: item.batches, remote: remoteBatches) {
                item.batches.removeAll(keepingCapacity: true)
    ...
    private func pendingSpotCheckItemIDs(
        organizationId: String,
        modelContext: ModelContext
    ) -> Set<UUID> {
        var descriptor = FetchDescriptor<PendingSyncAction>(
            predicate: #Predicate {
                ...
                $0.actionTypeRaw == "spot_check_set_count" &&
                $0.statusRaw != "synced"
            }
        )
        ...
        for action in pendingActions {
            if let payload = try? decoder.decode(ActionPayload.self, from: action.payloadData),
               case let .spotCheckSetCount(spotCheckPayload) = payload,
               let itemID = UUID(uuidString: spotCheckPayload.itemId)
            {
                itemIDs.insert(itemID)
                continue
            }
            if let refs = try? decoder.decode(AuditObjectRefs.self, from: action.refsData),
               let itemIDRaw = refs.itemId,
    ...
    private func fetchStoreScopedDocuments(
        ...
        for collectionName in collectionNames {
            if let storeRef = canonicalStoreRef,
    ...
    private func resolveCanonicalStoreDocumentReference(
        ...
        let cacheKey = "\(organizationId)|\(normalizedStoreID)"
        let now = Date()

        if let cached = canonicalStoreReferenceCache[cacheKey],
           now.timeIntervalSince(cached.cachedAt) <= canonicalStoreReferenceCacheTTL {
        ...
        if let groupedSnapshot = try? await db.collectionGroup("stores")
            .whereField("organizationId", isEqualTo: organizationId)
            .whereField(FieldPath.documentID(), isEqualTo: normalizedStoreID)
            .limit(to: 8)
            .getDocuments(),
           !groupedSnapshot.documents.isEmpty {
            let preferred = groupedSnapshot.documents.first {
                let path = $0.reference.path
                return path.contains("/regions/") && path.contains("/districts/")
            } ?? groupedSnapshot.documents[0]
            canonicalStoreReferenceCache[cacheKey] = (path: preferred.reference.path, cachedAt: now)
                ...
                if let districtsSnapshot = try? await regionDoc.reference.collection("districts").getDocuments() {
                    for districtDoc in districtsSnapshot.documents {
                        ...
                        if let storeDoc = try? await storeRef.getDocument(), storeDoc.exists {
                            canonicalStoreReferenceCache[cacheKey] = (path: storeRef.path, cachedAt: now)
                            return storeRef
                        }
                    }
                }
        ...
        if let legacyStoreDoc = try? await legacyStoreRef.getDocument(), legacyStoreDoc.exists {
            canonicalStoreReferenceCache[cacheKey] = (path: legacyStoreRef.path, cachedAt: now)
            return legacyStoreRef
        }

        canonicalStoreReferenceCache[cacheKey] = (path: nil, cachedAt: now)
        return nil
    }
    ...
    private func isSyncableOrganizationId(_ rawOrganizationId: String) -> Bool {
        let normalized = rawOrganizationId.trimmingCharacters(in: .whitespacesAndNewlines)
        guard !normalized.isEmpty else { return false }
        return normalized.lowercased() != "local-default"
    }
    ...
    private func upsertStoreBatches(
        ...
        var rowsToUpsert: [(id: String, payload: [String: Any])] = []
        for batch in item.batches {
            ...
            rowsToUpsert.append((id: backendID, payload: payload))
        ...
        var batchWriter = db.batch()
        var operationCount = 0

        func commitIfNeeded() async throws {
            guard operationCount >= 400 else { return }
            try await batchWriter.commit()
            batchWriter = db.batch()
            operationCount = 0
        }

        for row in rowsToUpsert {
            batchWriter.setData(row.payload, forDocument: baseRef.document(row.id), merge: true)
            operationCount += 1
            try await commitIfNeeded()
        }

        for doc in existingById.values where !keepIDs.contains(doc.documentID) {
            batchWriter.deleteDocument(doc.reference)
            operationCount += 1
            try await commitIfNeeded()
        }
    ...
    private func upsert(order: OrderItem, organizationId: String, db: Firestore) async throws {
        let normalizedStoreID = sanitizeStoreIdentifier(order.storeId)
        let ref = db.collection("organizations")
            .document(organizationId)
            .collection("orders")
        ...
        if !normalizedStoreID.isEmpty,
           let storeRef = await resolveCanonicalStoreDocumentReference(
            organizationId: organizationId,
            storeId: normalizedStoreID,
            db: db
           ) {
    ...
    private func upsert(todo: ToDoItem, organizationId: String, db: Firestore) async throws {
        let normalizedStoreID = sanitizeStoreIdentifier(todo.storeId)
        let ref = db.collection("organizations")
            .document(organizationId)
            .collection("toDo")
        ...
        if !normalizedStoreID.isEmpty,
           let storeRef = await resolveCanonicalStoreDocumentReference(
            ...
            let canonicalRef = storeRef
                .collection("toDo")
                .document(todo.id.uuidString)
            try await canonicalRef.setData(data, merge: true)
            try await storeRef
                .collection("todo")
                .document(todo.id.uuidString)
                .setData(data, merge: true)
    ...
    private func upsert(waste: WasteEntry, organizationId: String, db: Firestore) async throws {
        let normalizedStoreID = sanitizeStoreIdentifier(waste.storeId)
        let ref = db.collection("organizations")
            .document(organizationId)
            .collection("waste")
