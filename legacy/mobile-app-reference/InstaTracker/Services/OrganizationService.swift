...
final class OrganizationService {
    ...
    func stores(for organizationId: String) async throws -> [StoreLocationRef] {
        ...
        if firestoreEnabled {
            let db = Firestore.firestore()
            var storesByID: [String: (priority: Int, store: StoreLocationRef)] = [:]

            func mergeStore(_ store: StoreLocationRef, priority: Int) {
                if let existing = storesByID[store.id], existing.priority > priority {
                    return
                }
                storesByID[store.id] = (priority: priority, store: store)
            }

            if let groupedStores = try? await db.collectionGroup("stores")
                .whereField("organizationId", isEqualTo: organizationId)
                .limit(to: 2000)
                .getDocuments() {
                for storeDoc in groupedStores.documents {
                    guard let store = decodeStore(from: storeDoc.data(), id: storeDoc.documentID) else { continue }
                    let path = storeDoc.reference.path
                    let isNested = path.contains("/regions/") && path.contains("/districts/")
                    mergeStore(store, priority: isNested ? 2 : 1)
                }
            }
               ...
               let rootStoresSnapshot = try? await db.collection("organizations")
                ...
                for storeDoc in rootStoresSnapshot.documents {
                    if let store = decodeStore(from: storeDoc.data(), id: storeDoc.documentID) {
                        mergeStore(store, priority: 0)
                    }
                }
            }

            let stores = Array(storesByID.values.map(\.store))
                .sorted { $0.name.localizedCaseInsensitiveCompare($1.name) == .orderedAscending }
            if !stores.isEmpty {
