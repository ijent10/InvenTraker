...
enum CatalogInventoryImporter {
    ...
    static func importOrGetLocalItem(
        ...
        let resolvedUnitRaw = cleanedString(orgTemplate?.unit.rawValue) ?? cleanedString(record.unitRaw) ?? MeasurementUnit.each.rawValue
        let resolvedUnit = MeasurementUnit(rawValue: resolvedUnitRaw) ?? .each
